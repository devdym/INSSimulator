...
class ODIMThread extends Thread{
    private ODIMCubbyHole odcubbyhole;
    int p, winchN, leadinN, wtN;
    String msg;
    ...
    public void run(){
        while(done){
            ...
            try {
                ...
                Logger.getLogger(ODIMThread.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
    }
    
    public ODIMThread(ODIMCubbyHole c, int ODperiod, int winch, int leadin, int wt) {
        odcubbyhole = c;
        p = ODperiod;
        winchN = winch;
        leadinN = leadin;
    ...
    private String produceMsg() {
        ...
        winchN_hex = Integer.toHexString((winchN));
        if (winchN_hex.length()<2) {
            winchN_hex = "0" + winchN_hex;
        }
        
        leadinN_hex = Integer.toHexString((leadinN));
        if (leadinN_hex.length()<2) {
            leadinN_hex = "0" + leadinN_hex;
        }
        
        wtN_hex = Integer.toHexString((wtN));
        if (wtN_hex.length()<2) {
            wtN_hex = "0" + wtN_hex;
        }
        
        strN_hex = Integer.toHexString((winchN + leadinN + wtN));
